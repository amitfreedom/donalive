package com.mobile.donalive.agora.openvcall.ui.layout;
...
public class GridVideoViewContainer extends RecyclerView {
    ...
    private boolean initAdapter(Activity activity, int localUid, HashMap<Integer, SurfaceView> uids, GridVideoViewContainer mGridVideoViewContainer) {
        if (mGridVideoViewContainerAdapter == null) {
            mGridVideoViewContainerAdapter = new GridVideoViewContainerAdapter(activity, localUid, uids, mGridVideoViewContainer);
    ...
    public void initViewContainer(Activity activity, int localUid, HashMap<Integer, SurfaceView> uids, boolean isLandscape, GridVideoViewContainer mGridVideoViewContainer, int horizontal) {
        boolean newCreated = initAdapter(activity, localUid, uids, mGridVideoViewContainer);

        if (!newCreated) {
            mGridVideoViewContainerAdapter.setLocalUid(localUid);
            mGridVideoViewContainerAdapter.customizedInit(uids, true, mGridVideoViewContainer);
        }
        ...
        if (count <= 2) { // only local full view or or with one peer
            ...
            if (RecyclerView.HORIZONTAL == horizontal) {
                Toast.makeText(activity, "Horizontal", Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(activity, "Linear", Toast.LENGTH_SHORT).show();
